...
class ObservationMemory:
    ...
    def recall(self, query: str, n: int = 3, kind: str | None = None) -> list[dict]:
        ...
        try:
            ...
            if count > 0 and rows:
                query_vec = np.array(
                    self._embedder.encode_query([query])[0], dtype=np.float32
                )
    ...
    def format_for_context(self, memories: list[dict]) -> str:
        ...
        for m in memories:
            ...
            emotion_str = (
                f" [{m['emotion']}]"
                if m.get("emotion") and m["emotion"] != "neutral"
                else ""
            )
            lines.append(
    ...
    async def recall_async(
        self, query: str, n: int = 3, kind: str | None = None
    ) -> list[dict]:
    ...
    def format_day_summaries_for_context(self, summaries: list[dict]) -> str:
        if not summaries:
            return ""
        lines = ["[覚えていること — 過去の日々に見たもの、感じたこと]:"]
        for s in summaries:
    ...
    async def call(self, tool_name: str, tool_input: dict) -> tuple[str, str | None]:
        ...
        if tool_name == "recall":
            ...
            for m in memories:
                ...
                emotion = (
                    f" [{m['emotion']}]"
                    if m.get("emotion", "neutral") != "neutral"
                    else ""
                )
