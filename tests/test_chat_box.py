...
async def test_chat_box_success(mock_stdio_client_func, mock_ClientSession, mock_stdio_client, mock_mcp_session):
    ...
    mock_session.call_tool.assert_awaited_once_with(
        "send_message",
        arguments={"message": "Hello VRChat!"}
    )

@pytest.mark.asyncio
@patch("actuators.chat_box.ClientSession")
@patch("actuators.chat_box.stdio_client")
async def test_chat_box_tool_error(mock_stdio_client_func, mock_ClientSession, mock_stdio_client, mock_mcp_session):
    mock_session, mock_session_cm = mock_mcp_session
